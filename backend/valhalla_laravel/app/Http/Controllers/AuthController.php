...
{ 
    ...
    public function destroy($idCliente)
    {
        ...
        // 🔍 Validar si tiene reservas pendientes (futuras)
        $hoy = date('Y-m-d');
        $tieneReservasPendientes = DB::table('prestamo')
        ...
        if ($tieneReservasPendientes) {
            return response()->json([
                'message' => 'No puedes eliminar tu cuenta porque tienes reservas pendientes.'
            ], 409);
        }
    
        // ✅ Eliminar reservas futuras (si quedara alguna)
        DB::table('prestamo')
            ->where('id_cliente', $idCliente)
            ->where('fecha', '>=', $hoy)
            ->delete();
    
        // ✅ Desvincular reservas pasadas (mantener para historial, pero sin cliente)
        DB::table('prestamo')
            ->where('id_cliente', $idCliente)
            ->where('fecha', '<', $hoy)
            ->update(['id_cliente' => 50]);
    
        // ✅ Finalmente, eliminar el cliente
        $cliente->delete();
    
        return response()->json(['message' => 'Tu cuenta ha sido eliminada. Tus reservas pasadas se conservarán para fines de historial.'], 200);
    }
